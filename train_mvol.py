...
from utils import CIFAR10_Subset, Mixed_CIFAR100, RandomTinyImages300k
...
directory = "checkpoints/{exp_cat_name}/{in_dataset}/{auxiliary_dataset}/{model_arch}/{name}/".\
    format(exp_cat_name=args.exp_cat_name, auxiliary_dataset = args.auxiliary_dataset, in_dataset=args.in_dataset, model_arch= args.model_arch, name=args.name)
...
def main():
    ...
    if args.in_dataset == "CIFAR-10":
        ...
        normalizer = transforms.Normalize(mean=[x/255.0 for x in [125.3, 123.0, 113.9]],
                                         std=[x/255.0 for x in [63.0, 62.1, 66.7]])
        train_loader = torch.utils.data.DataLoader(
            CIFAR10_Subset('./datasets/cifar10', train=True, download=True, transform=transform_train),
            batch_size=args.batch_size, shuffle=True, **kwargs)
        val_loader = torch.utils.data.DataLoader(
    ...
    if args.auxiliary_dataset == 'RandomImages300k':
        ood_loader = torch.utils.data.DataLoader(RandomTinyImages300k(transform=transforms.Compose(
        ...
        Mixed_CIFAR100(cifar10_root ='../datasets/CIFAR-10', cifar100_root = '../datasets/CIFAR-100',transform=transforms.Compose(
            [transforms.ToTensor(), transforms.ToPILImage(), transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(), transforms.ToTensor()]), download=True, mixed_ritio=args.noise, 
            ), batch_size=args.ood_batch_size, shuffle=False,**kwargs
        )
    ...
    ood_criterion = MVOLLoss(args.factor).cuda()
...
def save_checkpoint(state, epoch):
    ...
    directory = "checkpoints/{exp_cat_name}/{in_dataset}/{auxiliary_dataset}/{model_arch}/{name}/"\
                 .format(exp_cat_name=args.exp_cat_name, in_dataset=args.in_dataset, auxiliary_dataset = args.auxiliary_dataset, model_arch= args.model_arch, name=args.name)
    if not os.path.exists(directory):
        os.makedirs(directory)
