...
from utils.custom_cifar_loader import KD_CIFAR10, KD_CIFAR100, KD_CIFAR10_Subset, CIFAR10_Subset
...
def loss_fn_kd(outputs, labels, logits, alpha = 0.5, temperature = 2):
    """
    ensemble distillation criterion
    """
    T = temperature
...
def main(): 
    ...
    if args.in_dataset == "CIFAR-10":
        ...
        teacher_train_loader = torch.utils.data.DataLoader(
            datasets.CIFAR10('./datasets/cifar10', train=True, download=True, transform=transform_test),
        ...
        normalizer = transforms.Normalize(mean=[x/255.0 for x in [125.3, 123.0, 113.9]],
                                         std=[x/255.0 for x in [63.0, 62.1, 66.7]])
        teacher_train_loader = torch.utils.data.DataLoader(
            datasets.CIFAR100('./datasets/cifar100', train=True, download=True,transform=transform_teacher_train),
            batch_size=args.inference_batch_size, shuffle=False, **kwargs)
        teacher_val_loader = torch.utils.data.DataLoader(
    ...
    teacher_val_outputs = generate_KD_label(teacher_loader=teacher_val_loader, teacher_model = teacher_model)
    teacher_train_outputs = generate_KD_label(teacher_loader = teacher_train_loader, teacher_model = teacher_model)
    ...
    if args.in_dataset == "CIFAR-10":
        ...
        student_train_loader = torch.utils.data.DataLoader(
            KD_CIFAR10('./datasets/cifar10', train=True, download=True, transform=transform_train,teacher_outputs=teacher_train_outputs), 
            batch_size=args.batch_size, shuffle=True, **kwargs)
        student_val_loader = torch.utils.data.DataLoader(
            KD_CIFAR10('./datasets/cifar10', train=False, transform=transform_test, teacher_outputs=teacher_val_outputs),
        ...
        student_train_loader = torch.utils.data.DataLoader(
            KD_CIFAR100('./datasets/cifar100', train=True, download=True,
                             transform=transform_train, teacher_outputs=teacher_train_outputs),
            batch_size=args.batch_size, shuffle=True, **kwargs)
        student_val_loader = torch.utils.data.DataLoader(
            KD_CIFAR100('./datasets/cifar100', train=False, transform=transform_test, teacher_outputs=teacher_val_outputs),
            batch_size=args.batch_size, shuffle=False, **kwargs)
    elif args.in_dataset =="CIFAR-10_subset":
        # Data loading code
        normalizer = transforms.Normalize(mean=[x/255.0 for x in [125.3, 123.0, 113.9]],
        ...
        student_train_loader = torch.utils.data.DataLoader(
            KD_CIFAR10_Subset('./datasets/cifar10', train=True, download=True,
                            ...
                            batch_size=args.batch_size, shuffle=True, **kwargs)
        student_val_loader = torch.utils.data.DataLoader(
            KD_CIFAR10_Subset('./datasets/cifar10', train=False, transform=transform_test, teacher_outputs=teacher_val_outputs),
                            batch_size=args.batch_size, shuffle=False, **kwargs)
    else:
        assert False, 'Not supported dataset: {}'.format(args.in_dataset)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(student_train_loader, model, criterion, optimizer, epoch)

        # evaluate on validation set
        validate(student_val_loader, model, criterion, epoch)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target, output_teacher_batch) in enumerate(train_loader):
        input = input.cuda()
        target = target.cuda()
    ...
    # log to TensorBoard
    if args.tensorboard:
        log_value('kd_train_loss', losses.avg, epoch)
        log_value('train_acc', top1.avg, epoch)
        log_value('train_loss', ce_losses.avg, epoch)
...
def validate(val_loader, model, criterion, epoch):
    ...
    # log to TensorBoard
    if args.tensorboard:
        log_value('kd_val_loss', losses.avg, epoch)
        log_value('val_acc', top1.avg, epoch)
        log_value('ce_val_loss', ce_losses.avg, epoch)
    return losses.avg, top1.avg
