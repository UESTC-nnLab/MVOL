...
keys = ['Gaussian Noise', 'Shot Noise', 'Impulse Noise', 'Defocus Blur', 'Glass Blur', 'Motion Blur', 'Zoom Blur', 'Snow', 'Frost', 'Fog', 'Brightness', 'Contrast', 'Elastic', 'Pixelate', 'JPEG', 'Speckle Noise', 'Gaussian Blur', 'Spatter', 'Saturate']

import multiprocessing
from joblib import Parallel, delayed

def process_one_img(img, severity_range):
    convert_img = trn.Compose([trn.ToPILImage()])
    transform_img = trn.ToTensor()
    ...
    for severity in severity_range:
        for method_name in d.keys():
            ...
            corruption_img = np.uint8(corruption(convert_img(img)))
            corruption_imgs.append(transform_img(corruption_img))

    return torch.stack(corruption_imgs, dim=0)

def gen_corruction_image(batch, severity_level = 2):
    batchs = []

    batch = batch.clone()

    severity_range = np.arange(1, severity_level + 1)

    corruption_imgs_list = Parallel(n_jobs=-1)(delayed(process_one_img)(img, severity_range) for img in batch)

    batchs = torch.stack(corruption_imgs_list, dim=0).transpose(0, 1)

    return batchs

def parallel_process_batch(batch, corruption):
    convert_img = trn.Compose([trn.ToPILImage()])
    transform_img = trn.ToTensor()
    ...
    for img in batch:
        corruption_img = np.uint8(corruption(convert_img(img)))
        imgs.append(transform_img(corruption_img))

    return torch.stack(imgs, dim=0)

def gen_random_corruction_image(batch, num_workers=4):
    batch = batch.clone()

    convert_img = trn.Compose([trn.ToPILImage()])
    transform_img = trn.ToTensor()

    random_severity = np.random.randint(2)+1
    random_method_name = keys[np.random.randint(19)]

    corruption = lambda clean_img: d[random_method_name](clean_img, random_severity)

    bs = int(batch.shape[0] / 4)

    imgs_list = Parallel(n_jobs=-1)(delayed(parallel_process_batch)(batch[i*bs:(i+1)*bs], corruption) for i in range(num_workers))
